...
abstract: Smart Pointers, a core concept in Rust, can seem cryptic and challenging to grasp. This post delves into the world of Smart Pointers, unraveling their intricacies and shedding light on their power in managing memory allocation and bound checking.
publishedOn: "2024-07-29T15:06:29+0530"
---

When I was first learning to code in rust, I remember finding the smart Pointers _extremely_ confusing. 😬

If you don't understand what it's doing, don't worry I got you.

In this blog post, we're going to learn how smart pointers work in rust.

First, let's start with pointers.

## What are Pointers in Rust?
...
## Then what are Smart Pointers in Rust and why do we need them?

Smart pointers basically is a way to do more things on top of references(`&`). They implement the `Deref` and `Drop` traits. Moreover, smart pointers can also have ownership of the data they are pointing to.
...
**Drop Trait:-** The Drop trait lets you decide what to do when a reference is going out of scope.
...
- When you have a type whose size is not defined,use Box.
...
**Rc** comes into the picture when you have multiple references to a memory but you are not sure in which order they gonna go out of scope. Rc counts the number of references to the memory and it keeps the references alive until the last reference goes out of scope.
...
- When your program is single threaded as Rc it is not thread-safe.
...
Atomic operations ensure that a set of operations on shared data is completed without interruption, providing thread safety and consistency.
...
So basically, Refcell implements runtime borrow checking, rather than compile-time checking like Rust’s usual borrowing rules. This allows certain memory-safe scenarios to be executed, which would have been disallowed by the compile-time checks.
...
The **Mutex** smart pointer is helpful when we want to be able to mutate shared data in multiple threads safely. It allows only one thread to access some data at any given time whether be it a writer or reader. It is used with Arc to ensure shared ownership among multi thread program.
...
Cow provides a flexible way of working with borrowed and owned data. It acts like an enum which lets you use both an owned instance of something or a borrowed instance using the same code.
...
- It should be used when you want you want to leverage efficient and smart memory allocation and data handling.
